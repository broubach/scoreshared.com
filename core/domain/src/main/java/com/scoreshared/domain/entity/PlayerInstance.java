...
@NamedQueries({
        ...
        @NamedQuery(name = "connectedPlayerInstancesByAssociationAndOutcomePracticeQuery", query = "select pi.id from PlayerInstance pi join pi.player p join p.invitation i left outer join pi.scoreLeft sl left outer join pi.scoreRight sr where p.association.id = :associationId and i.response = 0 and pi.approvalResponse = 0 and (sl.type = 2 or sr.type = 2)"),
        @NamedQuery(name = "connectedPlayerInstancesByAssociationAndOutcomeTieQuery", query = "select pi.id from PlayerInstance pi join pi.player p join p.invitation i left outer join pi.scoreLeft sl left outer join pi.scoreRight sr where p.association.id = :associationId and i.response = 0 and pi.approvalResponse = 0 and  (sl.type = 1 or sr.type = 1)"),
        @NamedQuery(name = "connectedPlayerInstancesByAssociationAndOutcomeWinQuery", query = "select pi.id from PlayerInstance pi join pi.player p join p.invitation i where p.association.id = :associationId and i.response = 0 and pi.approvalResponse = 0 and pi.scoreRight is null and pi.scoreLeft.type = 0"),
        @NamedQuery(name = "connectedPlayerInstancesByAssociationAndOutcomeLossQuery", query = "select pi.id from PlayerInstance pi join pi.player p join p.invitation i where p.association.id = :associationId and i.response = 0 and pi.approvalResponse = 0 and pi.scoreLeft is null and pi.scoreRight.type = 0"),
